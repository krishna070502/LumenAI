...
const SettingsSelect = ({
  ...
  const handleSave = async (newValue: any) => {
    ...
    setValue(newValue);
    try {
      // For theme, also update localStorage for immediate effect
      if (field.key === 'theme') {
        localStorage.setItem(field.key, newValue);
        setTheme(newValue);
      }

      // Save to per-user preferences API for preferences/personalization sections
      if (dataAdd === 'preferences' || dataAdd === 'personalization') {
        ...
        // For other sections (models, search), use global config (admin only)
  ...
  const handleSave = async (newValue: any) => {
    ...
    try {
      ...
      if (dataAdd === 'preferences' || dataAdd === 'personalization') {
        ...
        // For other sections, use global config
        const res = await fetch('/api/config', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
