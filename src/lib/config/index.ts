...
class ConfigManager {
  ...
  private initializeFromEnv() {
    ...
    /* Auto-complete setup when NVIDIA NIM is configured via environment */
    if (!this.currentConfig.setupComplete) {
      const nvidiaNim = this.currentConfig.modelProviders.find(
        (p) => p.type === 'nvidia-nim' && p.config?.apiKey
      );
      if (nvidiaNim) {
        console.log('NVIDIA NIM configured via environment - auto-completing setup');
