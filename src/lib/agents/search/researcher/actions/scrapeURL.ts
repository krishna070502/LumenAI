...
const scrapeURLAction: ResearchAction<typeof schema> = {
  ...
  execute: async (params, additionalConfig) => {
    // Defensive check: ensure urls is an array
    if (!params.urls || !Array.isArray(params.urls)) {
      params.urls = typeof params.urls === 'string' ? [params.urls] : [];
    }
    ...
    await Promise.all(
      params.urls.map(async (url) => {
        try {
          ...
          // Truncate content to prevent context length overflow (~20K chars per page)
          const maxContentLength = 20000;
          const truncatedContent = markdown.length > maxContentLength
            ? markdown.slice(0, maxContentLength) + '\n\n...[Content truncated due to length]'
            : markdown;
